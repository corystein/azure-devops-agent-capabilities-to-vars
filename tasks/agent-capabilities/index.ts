...
async function run() {
  try {
    ...
    switch (hostType) {
      ...
      case "deployment": {
        let deploymentGroupId: number = Number(
          task.getVariable("agent.deploymentGroupId")
        );
        ...
        let deploymentGroup = await agentApi.getDeploymentGroup(
          projectId,
          deploymentGroupId
        );
    ...
    try {
      ...
      throw new Error(
        "Invalid personal access token. Make sure the token is valid and active."
      );
    }

    console.log(
      "Agent capability variables and values. Format: variable=value"
    );
    console.log();
